...
HWND InitWindow(HINSTANCE hInstance,int nCmdShow)
{
	...
	if (screen_windowed == 0) {
		desk_hwnd=hWnd=CreateWindowEx(WS_EX_TOPMOST, "DDCWin", buf, WS_VISIBLE|WS_BORDER|WS_POPUP|WS_SYSMENU, 10, 10, 0, 0, NULL, NULL, hInstance, NULL);
	} else {
		desk_hwnd=hWnd=CreateWindowEx(0, "DDCWin", buf, WS_VISIBLE|WS_BORDER|WS_SYSMENU|WS_MINIMIZEBOX, 10, 10, screen_width+2, screen_height+21, NULL, NULL, hInstance, NULL);
	}
				   ...
				   LPSTR lpCmdLine, int nCmdShow)
{
	...
	if (mutex==NULL || GetLastError()==ERROR_ALREADY_EXISTS && strcmp(host_addr,"192.168.0.100")) {
